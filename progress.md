...
### 2026-07-20 - feat-090 Attendance production hardening across backend and frontend
- Backend check-in/check-out is now atomic and idempotent. It enforces staff assignment, ACTIVE campaign/date windows in Asia/Ho_Chi_Minh, valid paired GPS coordinates, one open attendance record, consistent event/campaign relationships, and stable soft-delete-aware queries. Notifications run after transaction commit so a notification failure cannot roll back attendance.
- Added eligible attendance targets, UTC/Instant API contracts, campaign-side filtering, manual correction reasons, before/after audit snapshots, explicit clear-field semantics, and soft delete. PostgreSQL now stores check-in/out as timestamp with time zone and applies 11 attendance constraints/indexes.
- Frontend now shows only assigned active campaigns/events, keeps a live in-shift timer, confirms check-out, exposes friendly retry/error states, refreshes safely after uncertain network outcomes, requires reasons for manager corrections/deletes, formats local time, and loads every attendance-report page instead of truncating after 200 rows.
- Verification: backend Docker image built/deployed and health is UP; 37/37 targeted backend tests passed; runtime covered eligible targets, idempotent replay, duplicate conflict, check-out replay, malformed payload 400, GPS validation, audit, 404 after soft delete, and left zero visible test records. Flutter attendance-targeted analysis has zero errors/warnings; full project analysis has no errors and one unrelated pre-existing warning (app_config.dart) plus info lints; release web build and Docker frontend build passed.
- Browser verification as staff@vnmap.local: selected an eligible campaign/event, checked in, observed the live in-shift state, opened and confirmed check-out, and returned to the ready state with a success snackbar. Browser console errors: 0. Evidence: output/attendance-ui-verification-20260720/.
- Residual reliability note: attendance data is transaction-safe, but push notification delivery is after-commit best effort rather than a durable outbox. Add an outbox/worker only if guaranteed delivery is a product requirement.

### 2026-07-20 - feat-091 Attendance Field Command Center UI
- Reworked staff/manager attendance into a responsive command-center layout: a prominent ready/active shift card, live duration, recent completed shifts, progressive note field, localized empty/loading/error states, and a refined team attendance table.
- Desktop uses a confirmation dialog; mobile uses a bottom sheet. Check-out now returns the completed record and shows an inline success panel with a working "View shift details" action. The previous false check-in success snackbar on DioException was removed.
- Responsive QA fixed two issues found during runtime testing: a duplicated mobile AppBar and encoding-sensitive date separators. At 390x844 the final page has one header, no horizontal overflow, touch-friendly controls, compact history, and bottom-sheet details.
- Verification: attendance-targeted Dart analyzer reported 0 errors and 0 warnings (13 info lints); full frontend analysis reported 217 existing issues, including two warnings outside the modified attendance UI. Flutter web release succeeded (main.dart.js 5,090,631 bytes), the artifact was loaded into vnmap_frontend and snapshotted as vnmap-fe:latest (bdf888c0d308), and backend health returned UP.
- Browser-tested two complete real check-in/check-out cycles as staff@vnmap.local across desktop and mobile, including cancel/confirm paths, success panel, history, and details. Console errors: 0. Four pre-existing assets/.env 404 warnings appeared across reloads. Test attendance #21 and #22 were then soft-deleted with an audit reason; visible records returned to 0. Evidence: output/attendance-ui-implementation-20260720/.
